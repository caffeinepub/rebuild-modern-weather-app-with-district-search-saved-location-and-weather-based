...
export function analyzeRouteWeather(
  weatherDataPoints: WeatherData[],
): RouteAnalysis {
  const factors: RouteFactor[] = [];
  let riskScore = 0;

  // Analyze freezing conditions across route
  const minTemps = weatherDataPoints.map((w) =>
    Math.min(...w.hourly.slice(0, 12).map((h) => h.temperature)),
  );
  ...
  const avgHumidity =
    weatherDataPoints.reduce((sum, w) => sum + w.current.humidity, 0) /
    weatherDataPoints.length;
  const hasFogCode = weatherDataPoints.some(
    (w) => w.current.weatherCode === 45 || w.current.weatherCode === 48,
  );
  ...
  const totalPrecip =
    weatherDataPoints.reduce((sum, w) => {
      return (
        sum + w.hourly.slice(0, 12).reduce((s, h) => s + h.precipitation, 0)
      );
  ...
  const maxWind = Math.max(
    ...weatherDataPoints.map((w) =>
      Math.max(...w.hourly.slice(0, 12).map((h) => h.windSpeed)),
    ),
  );
