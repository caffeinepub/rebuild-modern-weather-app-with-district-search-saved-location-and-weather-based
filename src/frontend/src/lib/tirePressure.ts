...
export function calculateTirePressureRecommendation(
  ...
  const basePressure =
    VEHICLE_BASE_PRESSURE[vehicleType as keyof typeof VEHICLE_BASE_PRESSURE] ||
    VEHICLE_BASE_PRESSURE.passenger;
  ...
  const _recommendedPressure = basePressure + pressureChange;
