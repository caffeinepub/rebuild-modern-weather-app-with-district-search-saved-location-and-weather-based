...
export function evaluateImminentWeatherAlerts(
  ...
  const hasStorm = nextHourData.some(
    (hour) =>
      hour.weatherCode === 95 ||
      hour.weatherCode === 96 ||
      hour.weatherCode === 99,
  ...
  if (hasStorm) {
    return {
      ...
      messageKey: "alert.imminent.storm.desc",
      severity: "danger",
    };
  }

  // Check for snow
  const hasSnow = nextHourData.some(
    (hour) =>
      hour.weatherCode === 71 ||
      hour.weatherCode === 73 ||
      hour.weatherCode === 75 ||
  ...
  if (hasSnow) {
    return {
      ...
      messageKey: "alert.imminent.snow.desc",
      severity: "warning",
    };
  }

  // Check for rain
  const hasRain = nextHourData.some(
    (hour) =>
      hour.weatherCode === 51 ||
      hour.weatherCode === 53 ||
      hour.weatherCode === 55 ||
  ...
  if (hasRain) {
    return {
      ...
      messageKey: "alert.imminent.rain.desc",
      severity: "info",
    };
  }

  // Check for fog (based on weather code or high humidity)
  const hasFog = nextHourData.some(
    (hour) =>
      hour.weatherCode === 45 ||
      hour.weatherCode === 48 ||
  ...
  if (hasFog) {
    return {
      ...
      messageKey: "alert.imminent.fog.desc",
      severity: "warning",
    };
  }
