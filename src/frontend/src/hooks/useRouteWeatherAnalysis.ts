...
import { type RouteAnalysis, analyzeRouteWeather } from "../lib/routeAnalysis";
import { type WeatherData, useWeather } from "./useWeather";
  ...
  return useQuery<RouteAnalysis>({
    ...
    queryFn: async () => {
      ...
      const weatherPromises = routePoints.map((point) =>
        fetchWeather(point.latitude, point.longitude).then(
